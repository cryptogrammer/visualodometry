...
classdef GenericEKF < handle
    ...
    methods
        ...
        % constructor
        function ekf = GenericEKF(robot, V_est, P0, varargin)
    ...
    methods (Access=protected)
        function x_est = step(ekf, opt)
            ...
            [odo, sensor_measurement] = ekf.robot.step();

            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %% do the prediction
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            % Vehicle  and map location and covariance
                ...
                Pvv_pred = Fx * Pvv_est * Fx' + Fv*odomCov*Fv'; %% <-- IMPLEMENT (use Fx, Pvv_est, odomCov);            
            ...
            if ~isempty(sensor_measurement)            
                % read the sensor
                [z,js] = ekf.sensor.reading(sensor_measurement);
            ...
            if doUpdatePhase
                ...
                S = Hx * P_pred * Hx' + Hw * ekf.W_est * Hw'; % <-- IMPLEMENT (use Hx, Hw, P_pred, ekf.W_est);

                % compute the Kalman gain
                K = P_pred * Hx' / S; % <-- IMPLEMENT (use P_pred, Hx, S);
